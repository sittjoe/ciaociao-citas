...
export async function GET(
  ...
  const fmtLocal = (dt: Date) => formatInTimeZone(dt, CDMX, "yyyyMMdd'T'HHmmss")
  ...
  const ics = [
    ...
    `DTSTAMP:${dtstamp}`,
    `DTSTART;TZID=${CDMX}:${fmtLocal(start)}`,
    `DTEND;TZID=${CDMX}:${fmtLocal(end)}`,
    ...
    `ATTENDEE;RSVP=TRUE;CN=${d.name}:mailto:${d.email}`,
    'STATUS:CONFIRMED',
    'END:VEVENT',
    'END:VCALENDAR',
  ].join('\r\n')
