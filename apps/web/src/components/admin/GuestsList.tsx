...
const STATUS_STYLES: Record<GuestStatus, string> = {
  ...
  excluded: 'bg-stone-100 text-stone-500 border border-stone-200',
}
...
export function GuestsList({ appointmentId }: GuestsListProps) {
  ...
  if (loading) {
    return <div className="text-xs text-ink-muted py-2">Cargando invitados…</div>
  }

  if (guests.length === 0) {
    return <div className="text-xs text-ink-muted py-2">Sin invitados en esta cita.</div>
  }

  return (
    <div className="space-y-2">
      {guests.map(g => (
        <div key={g.id} className="flex items-start justify-between gap-3 py-2 border-b border-stone-100 last:border-0">
